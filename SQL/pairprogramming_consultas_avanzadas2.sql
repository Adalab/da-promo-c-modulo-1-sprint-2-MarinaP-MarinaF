...
SELECT MAX(shipped_date) AS shipped_date, order_id AS order_id, employee_id AS employee_id
FROM orders
GROUP BY shipped_date, order_id, employee_id
ORDER BY
DAY(shipped_date), MONTH(shipped_date), YEAR(shipped_date);
